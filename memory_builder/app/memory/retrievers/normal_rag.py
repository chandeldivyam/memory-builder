...
class NormalRAG(Retriever):
    ...
    def retrieve(self, db, query_embedding) -> list[Node]:
        ...
        func.cosine_distance(Node.embedding, query_embedding_vector) < self.similarity_threshold).order_by('distance').limit(self.n_results).all()
